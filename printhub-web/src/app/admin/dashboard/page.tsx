...
async function handleDownload(fileUrl: string, fileName: string) {
  ...
  const url = fileUrl.startsWith('http') ? fileUrl : `${API_BASE.replace(/\/api\/v1$/, '')}${fileUrl}`;
  try {
                              ...
                              {order.items && order.items.map((item) => (
                                  <div key={item.id} className="flex items-center justify-between text-xs text-slate-700 bg-slate-50 px-3.5 py-2 rounded-xl border border-slate-100">
                                    ...
                                    <span className="flex items-center gap-3 text-slate-500">
                                      ...
                                      {item.fileUrl && (
                                        <button
                                          onClick={() => handleDownload(item.fileUrl!, item.fileName || 'document.pdf')}
                                          className="inline-flex items-center gap-1 px-2.5 py-1 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg font-semibold transition"
                                        >
